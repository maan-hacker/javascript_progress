...
const updateWeatherInfo = (result) => {
    ...
    if (error) {
        ...
        dateTime.innerText = `${current.temp_c} °C`;
        ...
        const codes = [
            [1000, 10000, 10001, 1100, 11000, 51190, 60030], // clear
            [
                1101, 11011, 11010, 11021, 11020, 1102, 1001, 10011, 1003, 1150, 1153, 1168, 1147, 1135, 1087, 1003, 1006, 1207, 1009,
            ], // cloudy
            [
                1261, 1273, 1276, 1274, 1246, 1243, 1240, 1201, 1204, 1198, 1192, 1195, 1189, 1186, 1183, 1180, 1186,
            ], // rainy
            [
                1030, 1066, 1168, 1171, 1210, 1213, 1216, 1219, 1222, 1225, 1237, 1255, 1258, 1279, 1282,
            ], // snowy
        ];
...
const getlocation = async (lat, long) => fetchData(
    `https://api.weather.com/v1/current.json?key=${apiKey}&q=${lat},${long}&aqi=no`,
);
